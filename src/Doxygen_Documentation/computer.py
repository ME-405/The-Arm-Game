'''!
@file    computer.py
@brief   This code goes on the front-end in order to communicate from the user controller to the micro-controller 
...
try:
    ...
    uart_to_micro.reset_input_buffer()				# Resets the buffer inputs
    uart_to_micro.reset_output_buffer()				# Resets the buffer outputs
    time.sleep(5)
    debug = False
...
while not done:
    ...
    for i in range(joystick_count):
        ...
        if Zaxis == 0:
            Zaxis = 0.000001
        if Xaxis == 0:  			# hardcore to never be at 0 to avoid division by zero
            Xaxis = 0.000001
        if Yaxis == 0:
            Yaxis = 0.000001
        if Zaxis == 0:
        ...
        if debug:
        ...
        elif ACK:  																# The Micro-controller is ready for another packet
            ...
            ACK = False  														# Wait for the Micro-controller to be ready for another packet
        else:  																	# check if the Micro-controller is ready for another packet
            temp_to_print = ""
