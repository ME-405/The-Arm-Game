'''!
@file    main.py
@brief   This is the main file for the Micro-python side of this project. It contains the FSM that controls the whole system.
...
class RoboticArm:
    def __init__(self, uart_channel=2, baudrate=115200):
        '''!
        @brief This class corresponds to the main file in which all other task are intercommunicating
...
# ---------------------------# now setup all of the class variables----------------------------------------------------------------------
	    ...
	    # This is the close loop controller for the base
        self.clc = ClosedLoop(EncoderDriver, Motor)  							# make a new closed loop controller
        # self.command = [1, 0.1, 0.1, 100, 60]  								#DEBUG
		
		# This is the raw command taken from the host computer, it will be processed into angles and positions at a later date
		...
		# This is a list that contains all of the angles for the servos to go to
        self.angles = [DEFAULTANGLE, DEFAULTANGLE, DEFAULTANGLE, DEFAULTANGLE]  # set all of the angles to default
		
		# This is the updated encoder endpoint for the base
        self.endpoint = DEFAULTANGLE  											# Set the default endpoint
		
		# This is a boolean to determine if we need to update the parameters
        self.new_values = False  												# we do not have any new values to read
        # self.mail = True 														# DEBUG
		
		# This is a boolean to determine if we have new mail from the computer
    ...
    def read_uart(self):
		'''!
		@brief It reads the values that are on the repository
		'''
    ...
    def calculate_parameters(self):
		'''!
		@brief It calculates the parameters in order to sent the signals to the motors and servos
		'''
    ...
    def update_parameters(self):
		'''!
		@brief Updates the given parameters based on the new desired location(s)
		'''
    ...
    def clc_passthrough(self):
		'''!
		@brief It runs the control algorithm function on closedLoopControl.py in order to calculate the actuation of the motor
		'''
